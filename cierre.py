...
st.set_page_config(page_title="CIERRE PRESUPUESTAL COPA", layout="wide")
...
archivos = [
    ...
    {
        "titulo": "Archivo EXCEL (Archivo Reservas):",
        "color": "red",
        "funcion_carga": cargar_excel,
        "parametro": "Cargar archivo XLSX (Archivo Reservas)",
...
# Procesar archivos si son cargados correctamente
if globals().get("df_ingresos") is not None and " FUENTE FINANCIACION" in globals()["df_ingresos"].columns:
    try:
        # Separar la columna ' FUENTE FINANCIACION' por el primer guion "-"
        columnas_separadas = globals()["df_ingresos"][" FUENTE FINANCIACION"].str.split("-", n=1, expand=True)

        # Extraer los números antes del primer guion para 'Parte_1'
        globals()["df_ingresos"]["Parte_1"] = columnas_separadas[0].str.extract(r'(\d+)', expand=False)

        # Asignar el resto del texto después del primer guion a 'Parte_2'
        globals()["df_ingresos"]["Parte_2"] = columnas_separadas[1] if columnas_separadas.shape[1] > 1 else None

        # Eliminar valores NaN o vacíos en 'Parte_1' si es necesario
        globals()["df_ingresos"]["Parte_1"].fillna('No Disponible', inplace=True)
