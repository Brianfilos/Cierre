...
df2 = cargar_csv2("Cargar archivo CSV (Ejecución Presupuestal Egresos)")

st.write("3. Archivo CSV (Ejecución Presupuestal - Ingresos)")
df3 = cargar_csv2("Cargar archivo CSV (Ejecución Presupuestal Ingresos)")

st.write("4. Archivo CSV (EJECUCION CONSOLIDADA DE INGRESOS Y GASTOS)")
df4 = cargar_csv2("Cargar archivo CSV (EJECUCION CONSOLIDADA DE INGRESOS Y GASTOS)")
...
df6 = cargar_csv2("Cargar archivo CSV (Reporte De Cuentas Por Pagar)")
...
if df3 is not None:  # Verifica que df3 haya sido cargado correctamente
    if " FUENTE FINANCIACION" in df3.columns:  # Verifica que la columna ' FUENTE FINANCIACION' exista
        try:
            ...
            columnas_separadas = df3[" FUENTE FINANCIACION"].str.split("-", n=1, expand=True)
            
            # Extraer solo los números antes del primer guion para 'Parte_1'
            df3["Parte_1"] = columnas_separadas[0].str.extract(r'(\d+)', expand=False)
            
            # Asignar el resto del texto después del primer guion a 'Parte_2'
            df3["Parte_2"] = columnas_separadas[1] if columnas_separadas.shape[1] > 1 else None  # Evitar errores si no hay suficientes partes
